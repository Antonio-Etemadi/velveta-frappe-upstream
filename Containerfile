...
# ----------------------------------------------------
# Upstream Frappe & ERPNext Builder Stage
# ----------------------------------------------------
...
RUN --mount=type=secret,id=github_pat,uid=1000,gid=1000,mode=0400 \
  ...
  find apps -mindepth 1 -maxdepth 1 -type d | sort | while read -r app_dir; do \
    if [ -d "$app_dir/.git" ]; then \
      ...
      app_url="$(git -C "$app_dir" config --get remote.origin.url || echo "")"; \
      app_commit="$(git -C "$app_dir" rev-parse HEAD || echo "")"; \
      echo "Installed upstream app: $app_name ($app_url @ $app_commit)"; \
    fi; \
  done

# ----------------------------------------------------
# Final Upstream Base Runtime Stage
# ----------------------------------------------------
FROM base AS final
